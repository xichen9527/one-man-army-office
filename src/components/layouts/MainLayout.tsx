...
import { Menu, X, Bell, User, ChevronDown, Search, LogOut, Settings, FileText, HelpCircle, RefreshCw, LayoutDashboard, MessageSquare, Share2, Video, Users, LayoutGrid, Bot } from 'lucide-react';
import { useStore } from '@/store';
import { supabase } from '@/db/supabase';
import { toast } from 'sonner';

// 先定义所有图标组件，避免 TDZ 错误
const WorkspaceIcon = ({ className }: { className?: string }) => <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 2 2 7l10 5 10-5-10-5z" /><path d="M2 17l10 5 10-5" /><path d="M2 12l10 5 10-5" /></svg>;
const ShieldIcon = ({ className }: { className?: string }) => <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z" /></svg>;

const navItems = [
  { path: '/dashboard', label: '工作台', icon: LayoutDashboard },
  { path: '/project-management', label: '项目管理中心', icon: LayoutGrid },
  ...
  { path: '/workspace', label: '工作空间', icon: WorkspaceIcon },
  { path: '/settings', label: '系统设置', icon: Settings },
  { path: '/admin', label: '管理面板', icon: ShieldIcon }
];
...
const iconMap: Record<string, React.ComponentType<{ className?: string }>> = {
  ...
  LayoutGrid: ({ className }: { className?: string }) => <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="3" width="18" height="18" rx="2" /><line x1="9" y1="3" x2="9" y2="21" /><line x1="15" y1="3" x2="15" y2="21" /></svg>,
  ...
  Users: ({ className }: { className?: string }) => <svg className={className} viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2" /><circle cx="12" cy="7" r="4" /></svg>,
